...
namespace ePrescription.Web.UI
{
	...
	public partial class FormUtilBase
	{
      ...
      public static void BindControl(Control control, IOrderedDictionary list, String propertyName)
      {
        ...
        #region Handles the GridViewMultiFilter State
        /// <summary>
        /// Handles the state of the GridViewMultiFilter
        /// </summary>
        /// <param name="multiFilter">The search panel.</param>
        public static void HandleGridViewMultiFilterState(GridViewMultiFilter multiFilter)
        {
            HandleGridViewMultiFilterState(multiFilter, PersistenceMethod.Session);
        }
        ...
        /// <param name="multiFilter">The search panel.</param>
        /// <param name="persistentCookieExpiryDateTime">The date and time the persistent cookie expires.</param>
        public static void HandleGridViewMultiFilterState(GridViewMultiFilter multiFilter, DateTime persistentCookieExpiryDateTime)
        {
            HandleGridViewMultiFilterState(multiFilter, null, PersistenceMethod.PersistentCookie, persistentCookieExpiryDateTime);
        }
        ...
        /// <param name="multiFilter">The search panel.</param>
        /// <param name="persistenceMethod">The persistence method.</param>
        public static void HandleGridViewMultiFilterState(GridViewMultiFilter multiFilter, PersistenceMethod persistenceMethod)
        {
            HandleGridViewMultiFilterState(multiFilter, null, persistenceMethod, null);
        }
        ...
        /// <param name="persistenceMethod">The persistence method</param>
        /// <param name="persistentCookieExpiryDateTime">The date and time the persistent cookie expires</param>
        public static void HandleGridViewMultiFilterState(GridViewMultiFilter multiFilter, GridView gridView,
                                                            PersistenceMethod persistenceMethod, DateTime? persistentCookieExpiryDateTime)
        {
            if (gridView == null)
            {
                // try and find the GridView control if it hasn't been explicitly supplied
                gridView = FindControl(multiFilter.NamingContainer, multiFilter.GridViewControlId) as GridView;
            }
            ...
            if (gridView != null)
            {
                ...
                gridView.Page.Load += new EventHandler(
                    delegate(object sender, EventArgs e)
                    {
                        if (!gridView.Page.IsPostBack)
                        {
                            GridViewMultiFilterState controlState = GetGridViewMultiFilterState(GetUniqueGridViewName(gridView), persistenceMethod);
                            if (controlState != null)
                            {
                                controlState.RestoreState(ref gridView, ref multiFilter);
                            }
                        }
                    }
                );
                ...
                //    delegate(object sender, GridViewCommandEventArgs e)
                //    {
                //        GridViewMultiFilterState controlState = new GridViewMultiFilterState(gridView, multiFilter);
                //        SaveGridViewMultiFilterState(controlState, GetUniqueGridViewName(gridView),
                //                                        persistenceMethod, persistentCookieExpiryDateTime);
                //        multiFilter.DataBind();
                //    }
                //);
                ...
                // save state when search id done
                multiFilter.SearchButtonClicked += new EventHandler(
                    delegate(object sender, EventArgs e)
                    {
                        GridViewMultiFilterState controlState = new GridViewMultiFilterState(gridView, multiFilter);
                        SaveGridViewMultiFilterState(controlState, GetUniqueGridViewName(gridView),
                ...
                // save state when reset is done
                multiFilter.ResetButtonClicked += new EventHandler(
                    delegate(object sender, EventArgs e)
                    {
                        GridViewMultiFilterState controlState = new GridViewMultiFilterState(gridView, multiFilter);
                        SaveGridViewMultiFilterState(controlState, GetUniqueGridViewName(gridView),
        ...
        /// <summary>
        /// Saves the GridViewMultiFilter State.
        /// </summary>
        /// <param name="controlState">State of the control.</param>
        /// <param name="controlStateName">Name of the control state.</param>
        /// <param name="persistenceMethod">The persistent method.</param>
        /// <param name="persistentCookieExpiryDateTime">The persistent cookie expiry datetime.</param>
        public static void SaveGridViewMultiFilterState(GridViewMultiFilterState controlState, string controlStateName,
                                                            PersistenceMethod persistenceMethod, DateTime? persistentCookieExpiryDateTime)
        {
            switch (persistenceMethod)
            {
                ...
                case PersistenceMethod.PersistentCookie:
                    #region Uses a client cookie
                    string serializedString = SerializeGridViewMultiFilterState(controlState);
                    HttpCookie cookie = new HttpCookie(controlStateName, HttpContext.Current.Server.UrlEncode(serializedString));
                    // Determine if this should be persisted longer than just the current session
                    if (persistentCookieExpiryDateTime != null)
                    {
        ...
        /// <summary>
        /// Gets the GridViewMultiFilter State from the cookie
        /// </summary>
        /// <param name="controlStateName">Name of the control state.</param>
        /// <param name="persistenceMethod">The persistent method.</param>
        /// <returns></returns>
        public static GridViewMultiFilterState GetGridViewMultiFilterState(string controlStateName, PersistenceMethod persistenceMethod)
        {
            GridViewMultiFilterState controlState = null;
            ...
            switch (persistenceMethod)
            {
                case PersistenceMethod.Session:
                    ...
                    if (HttpContext.Current.Session[controlStateName] != null)
                    {
                        controlState = HttpContext.Current.Session[controlStateName] as GridViewMultiFilterState;
                    }
                    #endregion
                    break;
                ...
                case PersistenceMethod.PersistentCookie:
                    ...
                    if (cookie != null)
                    {
                        controlState = DeserializeGridViewMultiFilterState(HttpContext.Current.Server.UrlDecode(cookie.Value));
                    }
                    #endregion
        ...
        /// <param name="controlState">The control state.</param>
        /// <returns></returns>
        private static string SerializeGridViewMultiFilterState(GridViewMultiFilterState controlState)
        {
            string serializedString = string.Empty;

            using (StringWriter stringWriter = new StringWriter())
            {
                XmlSerializer serializer = new XmlSerializer(typeof(GridViewMultiFilterState));
                serializer.Serialize(stringWriter, controlState);
                serializedString = stringWriter.ToString();
            }
        ...
        /// <param name="serializedString">The serialized string.</param>
        /// <returns></returns>
        private static GridViewMultiFilterState DeserializeGridViewMultiFilterState(string serializedString)
        {
            GridViewMultiFilterState controlState = null;
            XmlSerializer serializer = new XmlSerializer(typeof(GridViewMultiFilterState));
            controlState = (GridViewMultiFilterState)serializer.Deserialize(new StringReader(serializedString));
